...
<head>
	...
	<link href="{{asset('dist/css/style-' . session()->get('userLanguage.direction') . '.css')}}" rel="stylesheet" type="text/css">
    @else
				...
				<ul class="nav navbar-right top-nav pull-right">
                    ...
                    <li class="dropdown auth-drp" style="line-height: 66px;">
                        ...
                        <select class="selectpicker" id="setUserLanguage" data-style="btn-success btn-outline" >
                            @foreach(session()->get('systemLanguages') as $language)
                                ...
                                <option value="{{$language->id}}" @if($language->id == session()->get('userLanguage.id')) selected @endif>{{$language->name}}</option>
					...
					<li class="dropdown auth-drp">
						...
						<a href="#" class="dropdown-toggle pr-0" data-toggle="dropdown">
                            <img src=@if(Auth::user()->image) "{{asset(Auth::user()->image)}}" @else {{asset("dist/img/user1.png")}} @endif alt="user_auth" class="user-auth-img img-circle"/>
                        ...
                        <div class="panel-body  pa-0">
                            <div class="profile-box">
                                <div class="profile-cover-pic" style="min-height: 296px;max-height: 296px;">
                                    @if(Auth::user())
                                    <img src=@if(Auth::user()->cover_image) "{{asset(Auth::user()->cover_image)}}" @else {{asset("dist/img/cropper.jpg")}} @endif  style="height: 296px;width:100%;"/>
                                    <!--<div class="profile-image-overlay"></div>-->
                                    @elseif($order)
                                        @if($order->provider->user->cover_image)
                                    <img src=@if($order->provider->user->cover_image) "{{asset($order->provider->user->cover_image)}}" @else "" @endif  style="height: 296px;width:100%;"/>
                                        @endif
